...
install_skill() {
    ...
    if [[ -z "$skill_path" ]]; then
        echo -e "${RED}Skill '$skill_name' no encontrada${NC}"
        return 1
    fi
    ...
    if [[ ! -d "$source_path" ]]; then
        echo -e "${RED}Directorio fuente no existe: $source_path${NC}"
        return 1
    fi
...
install_all() {
    local target_dir=$1
    echo -e "${BLUE}Instalando todas las skills...${NC}"
    echo ""
    for skill in "${!SKILLS[@]}"; do
...
main() {
    ...
    target_dir="$(cd "$target_dir" 2>/dev/null && pwd)" || {
        echo -e "${RED}Directorio no existe: $1${NC}"
        exit 1
    }

    local claude_dir="$target_dir/.claude"

    echo -e "Directorio destino: ${BLUE}$target_dir${NC}"
    echo ""
    ...
    if [[ ! -d "$claude_dir" ]]; then
        ...
        echo -e "${GREEN}Directorio .claude/ ya existe${NC}"
        mkdir -p "$claude_dir/skills"
    fi
    echo ""
    ...
    if [[ $# -gt 1 ]]; then
        ...
        if [[ "$1" == "--all" ]] || [[ "$1" == "-a" ]]; then
            ...
            echo -e "${BLUE}Instalando skills especificadas...${NC}"
            echo ""
            for skill in "$@"; do
        ...
        if [[ "$selection" == "a" ]]; then
            ...
            echo ""
            echo -e "${BLUE}Instalando skills seleccionadas...${NC}"
            echo ""

            # Convertir selección a array de nombres
            local skill_names=("${!SKILLS[@]}")
            for num in $selection; do
                local index=$((num - 1))
                if [[ $index -ge 0 ]] && [[ $index -lt ${#skill_names[@]} ]]; then
    ...
    echo ""
    echo -e "Skills instaladas en: ${BLUE}$claude_dir/skills/${NC}"
}
